...
#[derive(Clone)]
pub enum OperatorArgValue<'a> {
    Plain(&'a [u8]),
    List(Vec<OperatorArg<'a>>),
}
...
struct ParsedArgsIter<'a> {
    args: &'a [OperatorArg<'a>],
    flag_offset: Option<usize>,
    flags_over: bool,
    positional_arg_idx: usize,
}

struct ParsedArgsIterWithBoundedPositionals<'a> {
    op_name: &'a str,
    full_span: Span,
...
impl<'a> OperatorArgValue<'a> {
    pub fn expect_plain(
        ...
        match self {
            OperatorArgValue::Plain(value) => Ok(value),
            OperatorArgValue::List(_) => Err(OperatorCreationError::new_s(
...
impl<'a> OperatorCallExpr<'a> {
    pub fn error_named_args_unsupported(
        &self,
        span: Span,
    ) -> OperatorCreationError {
        return OperatorCreationError::new_s(
            format!(
    ...
    pub fn require_at_most_one_param(
        &self,
        ...
        if self.args.len() != 1 {
            return Err(OperatorCreationError::new_s(
                format!(
                    "operator `{}` does not accept more than one parameter",
                    self.op_name
                ),
                self.span,
            ));
        ...
        match self.args[0].value {
            OperatorArgValue::Plain(value) => Ok(Some(value)),
            OperatorArgValue::List(_) => Err(OperatorCreationError::new_s(
                format!(
    ...
    pub fn require_single_string_param(
        &self,
    ) -> Result<&'a str, OperatorCreationError> {
        self.require_single_param()?.to_str().map_err(|_| {
            OperatorCreationError::new_s(
                format!(
                    "failed to parse `{}` parameter (invalid utf-8)",
                    self.op_name
                ),
                self.span,
            )
    ...
    pub fn require_single_usize_param(
        &self,
    ) -> Result<usize, OperatorCreationError> {
        self.require_single_number_param()
    }
    ...
    {
        let value = self.require_single_string_param()?;
    ...
    pub fn require_single_bool_arg_or_default(
        &self,
        default: bool,
        ...
        let val = self.require_single_param()?;
...
impl<'a> Iterator for ParsedArgsIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let OperatorArgValue::Plain(value) = first.value {
            ...
            if !self.flags_over && value.starts_with(&[b'-']) {
                    ...
                    value[1..].char_indices().next()
                {
                    ...
                    return Some(ParsedArg {
                        value: ParsedArgValue::Flag(
                            &value[begin + 1..end + 1],
                        ),
                        span: first.span.subslice(0, 1, 1, end + 1),
                    });
                }
            }
        }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.iter.next() {
            Some(arg) => match arg.value {
                ParsedArgValue::Flag(_) => Some(Ok(arg)),
                ParsedArgValue::NamedArg { .. } => Some(Ok(arg)),
